...
本发布无数据库迁移、无 systemd 单元变更（timer 保持 6 条 OnCalendar）。发布清单：

1. 独立测试环境执行完整 unittest 与 Ruff（含新增单库存轻量查询、maxdays 解析、批量组失败隔离用例；连续两次全量运行确认观测缓存无跨运行污染）。
2. `deploy/release.sh <归档> <完整SHA> --no-migrations` 正常发布。
3. 重启 Worker；观察一个完整 6 轮日：
   - 维护窗口延长：轻量轮每目标 1 次单库存请求（约 4 分钟/百目标/轮 + 18 次/分钟节流），深度轮不变；
   - QuotaUsage 日累计上升至约 3,600 credits/百目标（轻量 5×3×N + 深度 21×N）；
   - 验证新增交易保护物品在轻量轮次即出现在快照（`is_trade_protected`，感知延迟 ≤4 小时）；
   - 观察单库存端点失败率：失败任务按任务隔离（coverage=degraded），确认组内其余任务正常完成、批次最终 `completed_with_errors` 而非卡死；
   - 与供应商 `/account/me` 对照本地记账偏差。
