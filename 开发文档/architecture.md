...
- 定时扫描以北京时间日期与窗口序号组成唯一槽键 `{日期}-R0..R5`，每日 00/04/08/12/16/20 点各允许一个批次；R2 槽为深度轮（多源最大覆盖），其余 5 槽为轻量轮（batch 端点 + Steam 公开接口交叉）；重叠批次共享维护状态，最后一个批次结束后才恢复普通用户写操作。快照分层保留：48 小时内全部保留，其后每个（目标, 北京自然日）保留当日最后一张，整体保留 8 天；7 天自然日对比基线不受影响。
