...
mode=${3:---code-only}
[[ "$mode" == --code-only || "$mode" == --restore-database ]] || { echo "Invalid rollback mode" >&2; exit 2; }
test -d "$target"
test -d "$backup"
...
systemctl start cs2-inventory-schedule.timer cs2-inventory-cleanup.timer
